...
if page == "📊 EDA":
    ...
    with tab2:
        fig, ax = dfig(14, 5)
        ax.plot(df["time"], df["value"], color="#34d399", lw=1.5, label="Value")
        ax.set_title(
        ...
        st.markdown(
            '<div class="theory-box">This dataset combines a linear trend, multiple seasonal components, and Gaussian noise. LSTMs excel at learning these complex, long-term temporal dependencies that simpler models might miss.</div>',
            unsafe_allow_html=True,
        )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<span class="badge">Build & Train LSTM</span>', unsafe_allow_html=True)
    st.title("Build & Train Long Short-Term Memory Network")
    st.markdown(
        '<div class="theory-box">Configure the LSTM architecture. LSTMs use specialized gates to remember or forget information over long sequences, making them ideal for complex time series.</div>',
        unsafe_allow_html=True,
    )
    ...
    with c1:
        ...
        units = st.select_slider("LSTM Units", options=[16, 32, 64, 128, 256], value=64)
        layers_count = st.slider("Number of LSTM Layers", 1, 3, 1)
    ...
    with c2:
        if run_btn:
            try:
                ...
                ax.plot(hdf["val_loss"], color="#34d399", lw=2, label="Val")
            ...
            for i in range(layers_count):
                arch_lines += [
                    f"  LSTM({units}, return_sequences={'True' if i < layers_count - 1 else 'False'})"
                ]
                if i < layers_count - 1:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        '<div class="theory-box">The trained LSTM will forecast the next N steps based on the last known sequence. LSTMs are particularly good at maintaining context over longer forecast horizons.</div>',
        unsafe_allow_html=True,
    )
    ...
    try:
        ...
        if st.button("🔄 Generate Forecast", use_container_width=True):
            ...
            ax.set_title(f"{forecast_steps}-Step Ahead Forecast (LSTM)")
            ax.set_xlabel("Time Step")
            ax.set_ylabel("Value")
            ax.legend(labelcolor="white", facecolor=DARK_AX, edgecolor=GRID)
            plt.tight_layout()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.title("LSTM Architecture & Concepts")

    tab1, tab2 = st.tabs(["🔬 The LSTM Cell", "📉 Sequence Length Impact"])

    with tab1:
        st.markdown("**The Anatomy of an LSTM Cell**")
        st.markdown(
            '<div class="theory-box">Unlike a simple RNN, an LSTM cell maintains a <b>Cell State</b> (the "memory") that runs straight down the entire chain, with only minor linear interactions. Three gates regulate the flow of information:</div>',
            unsafe_allow_html=True,
        )
        ...
        |---|---|---|
        | **Forget Gate** | Decides what information to throw away from the cell state. | $f_t = \sigma(W_f \cdot [h_{t-1}, x_t] + b_f)$ |
        | **Input Gate** | Decides what new information to store in the cell state. | $i_t = \sigma(W_i \cdot [h_{t-1}, x_t] + b_i)$ |
        | **Cell State Update** | Updates the old cell state $C_{t-1}$ to $C_t$. | $C_t = f_t * C_{t-1} + i_t * \tilde{C}_t$ |
        | **Output Gate** | Decides what to output based on the cell state. | $o_t = \sigma(W_o \cdot [h_{t-1}, x_t] + b_o)$, $h_t = o_t * \tanh(C_t)$ |
        """)

        st.markdown("**Visualizing the LSTM Cell Flow**")
        fig, ax = dfig(14, 6)

        # Simple conceptual diagram of LSTM cell
        ax.set_xlim(0, 10)
        ax.set_ylim(0, 10)
        ax.axis("off")
        ...
        boxes = [
            ...
            (4, 7, 2, 1.5, "Forget Gate"),
            (4, 4, 2, 1.5, "Input Gate"),
            (7, 5.5, 2, 1.5, "Output Gate"),
            (7, 2, 2, 1.5, "Cell State $C_t$"),
        ...
        ax.plot(
            ...
            lw=4,
            zorder=1,
            label="Cell State (Memory)",
        )
        ax.text(0.5, 3.2, "C", color="#34d399", fontsize=12, fontweight="bold")
        ax.text(9.5, 3.2, "C", color="#34d399", fontsize=12, fontweight="bold")

        ax.set_title(
            "Conceptual LSTM Cell Architecture",
            color="#e2e8f0",
            fontsize=14,
            fontweight="bold",
        )
    ...
    with tab2:
        st.markdown("**Why Sequence Length Matters in LSTMs**")
        st.markdown(
            '<div class="theory-box">LSTMs are designed to handle <b>long-term dependencies</b>. If the sequence length is too short, the model cannot see the full seasonal pattern. If it is too long, it may overfit or require significantly more compute.</div>',
            unsafe_allow_html=True,
        )

        st.markdown("""
        - **Short Sequence (e.g., 5-10 steps)**: Model only sees immediate past. Good for high-frequency noise, bad for trend/seasonality.
        - **Optimal Sequence (e.g., 20-50 steps)**: Captures the full periodic cycle (seasonality) and recent trend direction.
        - **Very Long Sequence (e.g., 100+ steps)**: May capture macro trends, but increases training time and risk of overfitting without sufficient data.
        """)

st.markdown("---")
st.markdown(
    "<center style='color:#4a5568;font-size:.78rem'>Long Short-Term Memory (LSTM) Explorer &nbsp;|&nbsp; TensorFlow · Keras · Scikit-learn</center>",
    unsafe_allow_html=True,
)